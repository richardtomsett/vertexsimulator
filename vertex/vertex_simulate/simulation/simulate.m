  ...
  if comCount == 1
    ...
    for iSpk = 1:length(allSpike)
      ...
      for iPostGroup = 1:TP.numGroups
        % make iSpkSynroup an array containing indeces of multiple synapse
        % types per presynaptic neuron - mod this in
        % setupSynapseDynamicVars
        iSpkSynGroup = synMap{iPostGroup}(neuronInGroup(allSpike(iSpk)));
        for iType = 1:size(iSpkSynGroup, 2)
          if ~isempty(SynModel{iPostGroup, iSpkSynGroup(iType)})
            % Adjust time indeces according to circular buffer index
            tBufferLoc = synArr{allSpike(iSpk), 3} + ...
              SynModel{iPostGroup, iSpkSynGroup(iType)}.bufferCount - allSpikeTimes(iSpk);
            ...
            if sum(inGroup ~= 0)
              ...
              bufferIncomingSpikes( ...
                SynModel{iPostGroup, iSpkSynGroup(iType)}, ...
